...
COUNT=0
NUM=1
ls /logs/proxy*host* | xargs tail -f -q | grep -E "([0-9]{1,3}[\.]){3}[0-9]{1,3}" | while read line;
do
  ...
  then
    length=`echo $line | grep -o -m 1 -E "\[Length\ ([0-9]{1,})\]" | grep -m 1 -o '[[:digit:]]*'`
    domain=`echo ${line:0:80} | grep -m 1 -o -E "[a-z0-9]*\.[a-z0-9]*\.(de|net|org|com|website)|[a-z0-9]*\.(de|net|org|com|website)"`
#   echo "Home IP addresses checked are $HOME_IPS"
#   echo "Duration is $length"
#   echo "Incoming IP Address is $ipaddressnumber"
#   echo "Domain requested is $domain"
#   echo "Log file line is $line"
